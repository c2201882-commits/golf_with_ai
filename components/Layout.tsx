  ...
  return (
    <div className="flex flex-col h-full bg-gray-50">
      ...
      <div className={`fixed top-0 left-0 bottom-0 w-72 bg-white z-50 transform transition-transform duration-300 shadow-2xl flex flex-col ${isMenuOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <div className="flex flex-col flex-1 overflow-y-auto">
          ...
          <div className="p-4 space-y-2 flex-1">
            <button onClick={() => handleNav(() => dispatch({ type: 'SET_VIEW', payload: 'HOME' }))} className="w-full flex items-center space-x-3 p-3 bg-white hover:bg-gray-50 text-gray-700 rounded-xl font-semibold border border-gray-100 shadow-sm active:scale-95 transition-transform">
              <Grid size={20} className="text-gray-400" />
              <span>{t('mainMenu')}</span>
            </button>

            <button onClick={() => handleNav(() => dispatch({ type: 'SET_VIEW', payload: 'SOCIAL' }))} className="w-full flex items-center space-x-3 p-3 bg-white hover:bg-indigo-50 text-gray-700 rounded-xl font-semibold border border-gray-100 shadow-sm active:scale-95 transition-transform">
              <Users size={20} className="text-indigo-500" />
              <span>{t('socialHub')}</span>
            </button>

            <button onClick={() => handleNav(() => dispatch({ type: 'SET_VIEW', payload: 'PAST_GAMES' }))} className="w-full flex items-center space-x-3 p-3 bg-white hover:bg-gray-50 text-gray-700 rounded-xl font-semibold border border-gray-100 shadow-sm active:scale-95 transition-transform">
              <History size={20} className="text-orange-500" />
              <span>{t('pastGames')}</span>
            </button>

            <button onClick={toggleLanguage} className="w-full flex items-center space-x-3 p-3 bg-white hover:bg-gray-50 text-gray-700 rounded-xl font-semibold border border-gray-100 shadow-sm active:scale-95 transition-transform">
          ...
          <div className="p-4 text-center border-t border-gray-100 bg-gray-50 pb-safe-bottom shrink-0">
             <div className="text-xs text-gray-400 mb-2">Golf Master Pro v1.4</div>
             <a href="https://www.instagram.com/eric820709/" target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 text-xs font-bold text-pink-600 hover:text-pink-700 transition-colors bg-white px-3 py-1.5 rounded-full border border-pink-100 shadow-sm"><Instagram size={14} /> IG: eric820709</a>
          </div>
        </div>
      </div>
      ...
      <main className="flex-1 overflow-hidden relative">
        {isHome && (
            <button onClick={toggleMenu} className="absolute top-safe-top left-4 p-2 bg-white/80 backdrop-blur-sm rounded-full shadow-md z-20 text-gray-600" style={{ top: 'calc(1rem + env(safe-area-inset-top))' }}><Menu size={24} /></button>
        )}
        <div className="h-full overflow-y-auto no-scrollbar pb-safe-bottom">{children}</div>
      </main>
    </div>
  );
};